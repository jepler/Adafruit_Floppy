...
#define PUT_BIT(x) do { \
    ...
    // We do this by knowing that when we arrive, we are waiting to parse the final
    // '1' data bit of the MFM sync mark. This means we apply a special rule to the first
    // word, starting as though in the 'even' state but not recording the '1' bit.
